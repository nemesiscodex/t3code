...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's T3CODE_V2_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // Build an explicit copy of process.env minus T3CODE_V2_HOME (dev-runner
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "T3CODE_V2_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus T3CODE_V2_HOME; pass it
