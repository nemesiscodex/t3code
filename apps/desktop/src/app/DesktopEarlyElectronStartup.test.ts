...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { T3CODE_V2_HOME: "/home/user/.t3-test" },
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        T3CODE_V2_HOME: "/home/user/.t3-test",
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "t3code-v2-dev",
      linuxDesktopEntryName: "com.nemesiscodex.T3Code.V2.Development.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.t3-v2/dev when T3CODE_V2_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.t3-v2/dev/desktop-settings.json");
  ...
  it("treats whitespace-only T3CODE_V2_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        T3CODE_V2_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
