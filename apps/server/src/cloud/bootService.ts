...
const BOOT_SERVICE_NAME = "t3code-v2";
const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (com.nemesiscodex.t3code.v2), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "com.nemesiscodex.t3code.v2.service";
...
/**
 * Reads `T3CODE_V2_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=T3CODE_V2_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>T3CODE_V2_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=T3CODE_V2_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status t3code-v2.service`, then run `t3 service install`.";
