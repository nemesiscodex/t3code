 ...
 * own `.t3-v2` is checked first (matching dev-runner precedence); otherwise the
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.t3-v2`
    ...
    const envHome = yield* Config.string("T3CODE_V2_HOME").pipe(Config.option);
