...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".t3-v2"),
);
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3-v2 > ambient T3CODE_V2_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.T3CODE_V2_HOME = resolvedBaseDir;
    } else {
      delete output.T3CODE_V2_HOME;
    }
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // (gitignored) `.t3-v2` — see @t3tools/shared/devHome for why this must
    // outrank an ambient T3CODE_V2_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.T3CODE_V2_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.T3CODE_V2_HOME ?? (yield* DEFAULT_T3_HOME);
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit T3 Code data directory; runtime state is stored under userdata (equivalent to T3CODE_V2_HOME). Inside a git worktree this defaults to that worktree's own .t3-v2 so dev state stays off the shared home.",
    ),
    Flag.optional,
