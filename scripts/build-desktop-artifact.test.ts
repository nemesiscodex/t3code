...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code V2 (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code V2 (Nightly)");
  });

  it("switches desktop packaging icons to the nightly artwork for nightly versions", () => {
    assert.deepStrictEqual(resolveDesktopBuildIconAssets("0.0.17"), {
      macIconPng: "assets/v2/app-icon.png",
  ...
  it.effect("omits upstream update feeds for V2 builds", () =>
    Effect.gen(function* () {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code V2 (Alpha) 1.2.3 Installer",
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Code", schemes: ["t3code-v2", "t3code-v2-dev"] },
      ]);
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.nemesiscodex.t3code.v2",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.nemesiscodex.t3code.v2</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.nemesiscodex.t3code.v2");
